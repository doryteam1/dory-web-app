...
export class LightboxImagenComponent implements OnInit {
  ...
  constructor(private renderer: Renderer2) {
    this.renderer.listen('window', 'click', (e: Event) => {
       const lightboxImageContainsTarget =this.lightboxImage?.nativeElement.contains(e.target);
       const lightboxContainsTarget = this.lightbox?.nativeElement.contains(e.target);
