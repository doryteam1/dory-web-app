...
export class MisProductosComponent implements OnInit {
  ...
  addProducto() {
    ...
    let payload = Utilities.parseJwt(token!);
    let fileName =
      '/productos/' + 'prod-' + payload.sub + '-' + new Date().getTime();
    this.storage
      .cloudStorageTask(fileName, this.file)
      .percentageChanges()
      .subscribe((response) => {
  ...
  updateProducto() {
    ...
    if (!this.file) {
      ...
      this.proveedorService
        .updateProducto(
          newProducto,
          this.productos[this.itemUpdateIndex].codigo
        )
        .subscribe(
          (response) => {
            console.log(response);
            this.productos[this.itemUpdateIndex].nombreProducto =
              newProducto.nombreProducto;
            this.productos[this.itemUpdateIndex].descripcion =
      ...
      this.storage
        .refFromUrl(this.productImagePath)
        .put(this.file)
        .percentageChanges()
        .subscribe(
          (response) => {
                    ...
                    this.proveedorService
                      ...
                      .subscribe(
                        (response) => {
                          ...
                          this.productos[this.itemUpdateIndex].descripcion =
                            newProducto.descripcion;
                          this.productos[this.itemUpdateIndex].precio =
                            newProducto.precio;
                          this.file = null;
                          this.productImagePath = '';
                          this.modalService.dismissAll();
