...
export class MisVehiculosComponent implements OnInit {
 ...
 async addVehiculo() {
   ...
   try {
     const compressedFiles = await this.compressImageSizeService.handleImageUpload(this.file);
       let token = localStorage.getItem('token');
       let payload = Utilities.parseJwt(token!);
       let fileName =
         '/vehiculos/' + 'vehiculo-' + payload.sub + '-' + new Date().getTime();
      await this.storage
         .cloudStorageTask(fileName, compressedFiles)
         .percentageChanges()
         .subscribe((response) => {
  ...
  updateVehiculo() {
    ...
    if (!this.file) {
      ...
      this.vehiculosService
        .updateVehiculo(
          newVehiculo,
          this.vehiculos[this.itemUpdateIndex].id_vehiculo
        )
        .subscribe(
          (response) => {
            this.vehiculos[this.itemUpdateIndex].modelo = newVehiculo.modelo;
            this.vehiculos[this.itemUpdateIndex].capacidad =
              newVehiculo.capacidad;
            this.vehiculos[this.itemUpdateIndex].transporte_alimento =
              newVehiculo.transporte_alimento;
            this.file = null;
            this.vehiculoImagePath = '';
            this.modalService.dismissAll();
            this.loading = false;
      ...
      this.storage
        .refFromUrl(this.vehiculoImagePath)
        .put(this.file)
        .percentageChanges()
        .subscribe(
          (response) => {
                  ...
                  (downloadUrl) => {
                    ...
                    this.vehiculosService
                      ...
                      .subscribe(
                        (response) => {
                          console.log(response);
                          this.vehiculos[this.itemUpdateIndex].modelo =
                            newVehiculo.modelo;
                          this.vehiculos[this.itemUpdateIndex].capacidad =
                            newVehiculo.capacidad;
                          this.vehiculos[
                            this.itemUpdateIndex
                          ].transporte_alimento =
                            newVehiculo.transporte_alimento;
                          this.file = null;
                          this.vehiculoImagePath = '';
